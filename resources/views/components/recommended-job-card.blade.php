        ...
        <div class="card-body">	
            <div class="media mb-2">
                <div class="media-body">
                    <p class="mb-1">{{$recommendedJob->company->name}}</p>
                    <h4 class="fs-20 text-black">{{$recommendedJob->title}}</h4>
                </div>
            ...
            @php if($recommendedJob->salary_status == "public"): @endphp
                <span class="text-primary font-w500 d-block mb-3">{{ "₦".number_format($recommendedJob->salary_range->start_range, 2, '.', ',').' - '."₦".number_format($recommendedJob->salary_range->end_range, 0, '.', ',')}}</span>
            @php else: @endphp
